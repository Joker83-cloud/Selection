...
private class SofascoreFallbackClient {
    ...
    private data class Event(val home: String, val away: String, val homeGoals: Int, val awayGoals: Int)
    ...
    fun enrich(candidate: MatchCandidate): MatchCandidate {
        ...
        if (homePts != null && awayPts != null && homeRate != null && awayLossRate != null) return candidate

        val recovered = mutableListOf<String>()

        if (homePts == null || homeRate == null) {
            findTeam(candidate.home)?.let { team ->
                val events = loadLastEvents(team.id)
                if (homePts == null) pointsLastFive(candidate.home, events)?.let { homePts = it; recovered += "forma casa" }
                if (homeRate == null) homeWinRate(candidate.home, events)?.let { homeRate = it; recovered += "rendimento casa" }
            }
        }

        if (awayPts == null || awayLossRate == null) {
            findTeam(candidate.away)?.let { team ->
                val events = loadLastEvents(team.id)
                if (awayPts == null) pointsLastFive(candidate.away, events)?.let { awayPts = it; recovered += "forma ospite" }
                if (awayLossRate == null) awayLossRate(candidate.away, events)?.let { awayLossRate = it; recovered += "rendimento trasferta" }
            }
        }

        val note = if (recovered.isEmpty()) {
            "Fallback SofaScore: nessun dato aggiuntivo recuperato"
        } else {
            "Fallback SofaScore · recuperati: ${recovered.distinct().joinToString(", ")}"
        }
    ...
    private fun parseEvents(arr: JSONArray, out: MutableList<Event>) {
        for (i in 0 until arr.length()) {
            ...
            val hg = scoreValue(e.optJSONObject("homeScore"))
            val ag = scoreValue(e.optJSONObject("awayScore"))
            if (home.isNotBlank() && away.isNotBlank() && hg != null && ag != null) out += Event(home, away, hg, ag)
        }
    }
